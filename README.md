...
This repository contains the frozen architecture baseline, the complete SCL Phase 1 core networking foundation, the complete Phase 2 Android video pipeline through RFC-002G, the complete Phase 3 audio pipeline through RFC-003H, and the Phase 4 reverse-input foundation through RFC-004D.
...
- Portable Input Payload to Android injection mapping, reverse-input receive orchestration, and end-to-end reverse input.
...
Phase 4 reverse input has begun. RFC-004A defines Portable Input Payload Version 1 for keyboard, multi-touch, absolute pointer, relative pointer, scroll, common gamepad state, and reset messages without embedding Android-specific identifiers or pixel coordinates into the protocol. RFC-004B adds an Android input-capture foundation above that model: hardware keyboard, touchscreen, mouse/pointer, scroll, and common game-controller events delivered to the Warpnect input surface can be translated into portable Warpnect input models with monotonic source event timestamps and session-local logical device slots. RFC-004C adds immediate reverse SCL transport: one portable observation is serialized directly into one bounded UDP datagram through JNI, without a sender queue, retry policy, batching, input fragmentation, or input-specific reliability mechanism. RFC-004D adds a privileged Android injection foundation using a Shizuku/Sui UserService and direct InputManager asynchronous submission. Portable input mapping and continuous reverse-input receive orchestration remain deferred.
